...
// Assuming that `F` is a field type like `Fq` (from Arkworks) or another field in your code.
type F = Fq; 
...
fn main() {
    ...
    let secret: u64 = secret_input.trim().parse().expect("Please enter a valid number");
    ...
    let total_shares: u64 = total_shares_input.trim().parse().expect("Please enter a valid number");
    ...
    let threshold: u64 = threshold_input.trim().parse().expect("Please enter a valid number");
